...
def load_logs():
    if not os.path.exists("data"):
        os.makedirs("data")
    if not os.path.exists("data/carlbot_logs.json"):
        return {}
    with open("data/carlbot_logs.json", "r") as f:
        return json.load(f)

def save_logs(data):
    with open("data/carlbot_logs.json", "w") as f:
...
class Logs(commands.Cog):
    ...
    async def on_member_update(self, before, after):
        if before.roles != after.roles:
            ch = await self.get_log_channel(after.guild)
            if ch:
                added = [r for r in after.roles if r not in before.roles]
                removed = [r for r in before.roles if r not in after.roles]
                embed = discord.Embed(title="🎭 تغيير الرتب", color=discord.Color.purple())
                embed.add_field(name="العضو", value=after.mention, inline=True)
                if added:
                    embed.add_field(name="رتب مضافة", value=", ".join([r.mention for r in added]), inline=False)
                if removed:
                    embed.add_field(name="رتب محذوفة", value=", ".join([r.mention for r in removed]), inline=False)
                await ch.send(embed=embed)
